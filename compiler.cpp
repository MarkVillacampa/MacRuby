	...
	Value **args_end)
{
    ...
    llvm::ArrayRef<Value *> params(args_begin, args_end);
	return CallInst::Create(imp, params, "", bb);
    }
    else {
	BasicBlock *normal_bb = BasicBlock::Create(context, "normal",
		bb->getParent());
    llvm::ArrayRef<llvm::Value *> params(args_begin, args_end);
	InvokeInst *dispatch = InvokeInst::Create(imp, normal_bb,
		rescue_invoke_bb, params, "", bb);
	bb = normal_bb;
	return dispatch;
    }
}
	...
	const size_t len)
{
    ...
    return GetElementPtrInst::Create(gvar, idxs, "", bb);
}
	...
	NODE *body)
{
    ...
    CallInst::Create(prepareMethodFunc, args, "", bb);
}
	...
	bool singleton, Function *func, rb_vm_arity_t &arity, NODE *body)
{
    ...
    CallInst::Create(prepareMethodFunc, params, "", bb);
}
...
RoxorCompiler::attach_current_line_metadata(Instruction *insn)
{
	...
	DILocation loc = DILocation(MDNode::get(context, args));
#else
...
RoxorCompiler::compile_multiple_assignment(NODE *node, Value *val)
{
	...
	Value *elt = CallInst::Create(masgnGetSplatFunc, args,
		"", bb);
...
RoxorCompiler::compile_multiple_assignment(NODE *node)
{
	...
	CallInst *ary = CallInst::Create(newArrayFunc, args, "", bb);
...
RoxorCompiler::compile_prepare_block(void)
{
    ...
    return CallInst::Create(prepareBlockFunc, params,
	    "", bb);
}
...
RoxorCompiler::compile_binding(void)
{
    ...
    return CallInst::Create(pushBindingFunc, params,
	    "", bb);
}
...
RoxorCompiler::compile_ivar_get(ID vid)
{
    ...
    return CallInst::Create(getIvarFunc, args, "", bb);
}
...
RoxorCompiler::compile_ivar_assignment(ID vid, Value *val)
{
    ...
    CallInst::Create(setIvarFunc, args, "", bb);
...
RoxorCompiler::compile_cvar_assignment(ID name, Value *val)
{
    ...
    return CallInst::Create(cvarSetFunc, args, "", bb);
}
...
RoxorCompiler::compile_constant_declaration(NODE *node, Value *val)
{
    ...
    CallInst::Create(setConstFunc, args, "", bb);
...
RoxorCompiler::compile_dstr(NODE *node)
{
    ...
    return CallInst::Create(newStringFunc, params, "",
	    bb);
}
...
RoxorCompiler::compile_return_from_block_handler(int id)
{
    ...
    Value *val = CallInst::Create(checkReturnFromBlockFunc, args,
	    "", bb);
...
RoxorCompiler::compile_landing_pad_header(void)
{
    ...
    params.clear();
    params.push_back(exn_ptr);
    return CallInst::Create(beginCatchFunc, params,
        "", bb);
}
...
RoxorCompiler::compile_literal(VALUE val)
{
	...
	if (rb_str_chars_len(val) == 0) {
	    ...
	    return CallInst::Create(newString2Func, args, "", bb);
	}
    }
...
RoxorCompiler::compile_set_current_scope(Value *klass, Value *scope)
{
    ...
    CallInst::Create(setScopeFunc, args, "", bb);
}
...
RoxorCompiler::compile_keep_vars(BasicBlock *startBB, BasicBlock *mergeBB)
{
    ...
    CallInst::Create(keepVarsFunc, params, "", bb);
...
rescan_args:
	...
	if (f->arg_size() - 2 == argc) {
	    ...
	    CallInst *inst = CallInst::Create(f, params,
		    "", bb);
...
RoxorCompiler::compile_node0(NODE *node)
{
	...
	case NODE_VALUES:
		...
		return CallInst::Create(newArrayFunc, args, "", bb);
	    }
	    break;
	...
	case NODE_HASH:
		...
		if (node->nd_head != NULL) {
			...
			CallInst::Create(storeHashFunc, args, "", bb);
		    }
		}
	...
	case NODE_VALIAS:
		...
		CallInst::Create(valiasFunc, args, "", bb);
...
RoxorAOTCompiler::compile_init_function(void)
{
	...
	switch (TYPE(val)) {
	    ...
	    case T_REGEXP:
		{
		    ...
		    lit_val = CallInst::Create(newRegexp2Func, args,
			    "", bb);
		}
		break;
	...
	try {
	    ...
	    CallInst::Create(addStub, args, "", bb);
	}
	catch (...) {}
    }
...
RoxorCompiler::compile_write_attr(ID name)
{
	...
	CallInst::Create(willChangeValueFunc, "", bb);
	...
	CallInst::Create(didChangeValueFunc, args, "", bb);
    }
    else {
	val = compile_ivar_assignment(name, new_val);
    }
					 ...
					 rb_vm_bs_boxed_t *bs_boxed)
{
    ...
    CallInst::Create(getStructFieldsFunc, args, "", bb);
}
	...
	Value *slot)
{
	...
	case _C_STRUCT_B:
		...
		CallInst::Create(initBlockFunc, args, "", bb);
	...
	case _C_ARY_B:
		...
		val = CallInst::Create(checkArrayFunc, args, "", bb);
...
RoxorCompiler::compile_new_struct(Value *klass, std::vector<Value *> &fields)
{
    ...
    return CallInst::Create(newStructFunc, fields,
	    "", bb);
}
...
RoxorCompiler::compile_new_opaque(Value *klass, Value *val)
{
    ...
    return CallInst::Create(newOpaqueFunc, args, "", bb);
}
...
RoxorCompiler::compile_new_pointer(const char *type, Value *val)
{
    ...
    return CallInst::Create(newPointerFunc, args, "", bb);
}
	...
	case _C_STRUCT_B:
		...
		if (bs_boxed != NULL) {
			...
			for (unsigned i = 0; i < bs_boxed->as.s->fields_count;
			    ...
			    const char *ftype = bs_boxed->as.s->fields[i].type;
			    Type *llvm_ftype = convert_type(ftype);
	...
	bool is_objc)
{
    ...
    // Compile call.
    CallInst *imp_call = CallInst::Create(imp, params,
	    "", bb);
...
RoxorCompiler::compile_lvar_assignment(ID vid, Value *val)
{
	...
	return CallInst::Create(writeBarrierFunc, args, "", bb);
    }
    else {
	new StoreInst(val, slot, bb);
    }
...
RoxorCompiler::compile_mri_stub(void *imp, const int arity)
{
    ...
    CallInst::Create(setCurrentMRIMethodContext, args, "", bb);
    ...
    CallInst *imp_call = CallInst::Create(imp_val, params, "", bb);
